...
{
    gk::Color color = gk::Color(0.0, 0.0, 0.0);
    int nbSamples = 5;
    for (unsigned int i = 0; i < sources.size(); i++){ 
		for (int j = 0; j < nbSamples; j++){
			gk::Point pSource;
			float proba = sources[i].triangle.sampleUniform(oneFloat0to1(), oneFloat0to1(), pSource);
			gk::Ray raySource(p, pSource);
			gk::Hit hitSource(raySource);
			
			if (!(intersect(raySource, hitSource))){
				color +=  gk::Color(material.diffuse_color) * -gk::Dot(n, gk::Normalize(p - pSource))/nbSamples;
			}
		}
    }
    return color;
