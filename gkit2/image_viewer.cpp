...
public:
    ...
    TP( char **_filenames, const int n )
        :
        gk::App(),
    ...
    int init( )
    {
        ...
        for(int i= 0; i < m_count; i++)
        {
            ...
            if(gk::IOFileSystem::isFilename(file) == false) continue;
            
            gk::Image *image= gk::readImage(file);
            if(m_images->can_pack(image))
                m_images->push_back(image);
        }
        
        if(m_images->images.size() == 0) return -1;
        ...
        for(int y= 0; y < m_image->height; y++)
            ...
            {
                const gk::Vec4 &pixel= m_image->pixel(x, y);
        ...
        m_bins_min= (float) (m_image->width * m_image->height);
        m_bins_max= 0;
        float bins_sum= 0.f;
    ...
    int draw( )
    {
        ...
        if(m_show_histogram)
        {
            ...
            m_color_program->uniform("color")= gk::Vec4(1, 0, 0, 1);
